...
#End If
    ...
    If Not EntryProcIsKnown Or ErrBttns(err_buttons) = 1 Then
        ...
        If bRegression Then
            ...
            Then vErrReply = ErrMsgDsply(err_source:=sInitErrSource, err_number:=lInitErrNo, err_dscrptn:=sInitErrDscrptn, err_line:=lInitErrLine, err_buttons:=err_buttons)
        Else
            vErrReply = ErrMsgDsply(err_source:=sInitErrSource, err_number:=lInitErrNo, err_dscrptn:=sInitErrDscrptn, err_line:=lInitErrLine, err_buttons:=err_buttons)
        End If
        ErrMsg = vErrReply
        err_reply = vErrReply
...
#End If
    ...
    ElseIf Not ErrIsAsserted(lInitErrNo) Then
